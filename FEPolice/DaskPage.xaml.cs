...
public partial class DaskPage : ContentPage
{
    ...
    public DaskPage(IRestDataService dataService, Person person, int productid)
    {
        InitializeComponent();
    ...
    protected async Task OnAppearing(Person person, int productid)
    {
        ...
        {
            if (matchedPolicies[i].Discriminator == "Dask")
            {
    ...
    {
        await Navigation.PushAsync(new AddDaskPolicy(_dataService, p, productid));
