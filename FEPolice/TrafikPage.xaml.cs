...
public partial class TrafikPage : ContentPage
{
    ...
    int productid;
    public TrafikPage(IRestDataService dataService, Person person, int productid)
    {
        InitializeComponent();
    ...
    protected async Task OnAppearing(Person person, int productid)
    {
        ...
        {
            if (matchedPolicies[i].Discriminator == "Traffic")
            {
    ...
    {
        await Navigation.PushAsync(new AddTrafikPolicy(_dataService, p, productid));
